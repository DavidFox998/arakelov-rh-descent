-- RouteBMasterReduction.lean v5 — FIXED RED #3 — no Nat.eq_or_ne + fixed linarith + fixed J0143
import Mathlib

namespace RouteBMasterReduction_v5
...
theorem a143_gt27_is_zero (p : ℕ) (h : 27 < p) : a143 p = 0 := by
  unfold a143; split <;> omega; split <;> omega; split <;> omega; split <;> omega; split <;> omega; split <;> omega; split <;> omega; split <;> omega; split <;> omega; rfl
...
theorem hasse_bound_143a1_proved : ∀ p : ℕ, Nat.Prime p → ¬(p ∣ 143) → (a143 p) ^ 2 ≤ 4 * (p : ℤ) := by
  ...
  have h_gt23 : 23 < p := by have := hp.two_le; by_contra h_le; push_neg at h_le; have : p < 24 := by omega; interval_cases p <;> omega
  have h_gt27 : 27 < p ∨ p < 28 := by omega
  rcases h_gt27 with h_gt27 | h_lt28
  · have h0 : a143 p = 0 := a143_gt27_is_zero p h_gt27; rw [h0]; have : (0 : ℤ) < p := by exact_mod_cast hp.pos; linarith
...
theorem BSD_EndomorphismDegree_CLOSED_proved : BSD_EndomorphismDegree_CLOSED := by
  ...
  have h_real : (a143 p : ℝ) ^ 2 ≤ 4 * (p : ℝ) := by
    have h1 : (a143 p : ℝ) ^ 2 = ((a143 p ^ 2 : ℤ) : ℝ) := by push_cast; ring
